...
public record TicketCreateDTO(
        ...
        @Schema(description = "The priority of the ticket.", examples = {"LOW", "MEDIUM", "HIGH", "CRITICAL"})
        @NotBlank(message = "Priority is a required field.")
        @Pattern(regexp = "^(LOW|MEDIUM|HIGH|CRITICAL)$")
        String priority,

        @Schema(description = "The status of the ticket.", examples = {"OPEN", "ON_GOING", "CLOSED"})
        @NotBlank(message = "Status is a required field.")
        @Pattern(regexp = "^(OPEN|ON_GOING|CLOSED)$")
        String status,

        @Schema(description = "The expiry date of the ticket.")
